...
//        T t = JsonFastJsonUtil.parse(json, tClass);
    ...
    public static <R extends Serializable> NullChain<R> merge(Class<R> tClass, List<NullChain<?>> nullChains) {
        ...
        try {
            ...
            for (NullChain<?> nullChain : nullChains) {
                if (!nullChain.is()) {
                    BeanCopyUtil.copyIgnoreNull(nullChain.get(), r);
                }
            }
            return NULL.of(r);
    ...
    //添加头部日志定位
    public  static  void addHeadLog(StringBuffer linkLog){
        String stackTraceString = StackTraceUtil.currentStackTraceString(4);
        //添加头部
        linkLog.insert(0,stackTraceString);
    }
