...
public class RadixSortUtil {
     ...
     * 注意事项:  数组内的值必须位数一致 比如都是4位  都是3位  不能出现 混合情况
     *
     * @param integers 待排序整数数组
     * @param d        待排序数组中整数的位数
     */
    public static void radixSort(Integer[] integers, int d) {
        /**
     ...
     * @return
     */
    public static Integer getValueByIndex(Integer integer, int index) {
        ...
        // 如果是负数符号的话,那么就往后去
        if ("-".equals(value)) {
           value = iString.substring(index+1, index + 2);
        }
