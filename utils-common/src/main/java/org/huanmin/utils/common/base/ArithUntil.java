...
public class ArithUntil { //默认除法运算精度
    private static final int DEF_DIV_SCALE = 10;   //默认小数位最大为10
    private ArithUntil(){//这个类不能实例化
    }
    /**
     * 提供精确的加法运算。
     * @param v1 被加数
     ...
     * @param v1 被除数
     * @param v2 除数
     * @param scale 表示表示需要精确到小数点以后几位。(必须有否则会出现未知错误)
     * @return 两个参数的商
     */
     ...
     * 提供精确的小数位四舍五入处理。
     * @param v 需要四舍五入的数字
     * @param scale 小数点后保留几位  (必须有否则会出现未知错误)
     * @return 四舍五入后的结果
     */
    ...
    /**
     * toString() – 在必要的时候使用科学计数法 (自动适配)
     *
     */
    public static String   turn(double v){
        return  new BigDecimal(v).toString();
    }
    /**
     * 将科学计数法 转为Spring普通数字  toPlainString() – 不使用任何科学计数法。
     * @param v
     */
    public static String   turnPlain(double v){
