...
/**
 * https://github.com/DiUS/java-faker  各种数据模拟
 */
public class FakerData {
    ...
    public static int getAge() {
        return new Random().nextInt(42) + 18;
    }
    /**
     * 随机生成指定数量的double值
     *
     ...
     * @return double[]
     */
    public static double[] getDataDouble(int size) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            String format1 = df.format(ra);
            double v = Double.parseDouble(format1);
            i1 += v;
            int i2 = random.nextInt(length);
            String format2 = df.format(ra);
            double v2 = Double.parseDouble(format2);
            i2 += v2;
            arr[i] = i1 - i2;
        }
     ...
     * @return int[]
     */
    public static int[] getDataInt(int size) {
     ...
     * @return int[]
     */
    public static int[] getDataIntPositive(int size) {
        int[] arr = new int[size];
        int length = arr.length;
        Random random = new Random();
        for (int i = 0; i < length; i++) {
     ...
     * @return int[]
     */
    public static Integer[] getDataInteger(int size) {
     ...
     *
     * @param size 数量
     * @param  range  数据范围   0~range
     * @return int[]
     */
    public static Integer[] getDataInt(int size,int range) {
        Integer[] arr = new Integer[size];
        int length = arr.length;
        Random random = new Random();
        for (int i = 0; i < length; i++) {
    ...
    /**
     * 随机生成指定数量的int值 ,并且控制位数
     *
     * @param size 数量
     * @param  number 多少位     比如:4位  那么生成的数据范围 1000~9999
     * @return int[]
     */
    public static Integer[] getDataIntNumber(int size, int number) {
        Integer[] arr = new Integer[size];
        int length = arr.length;
        ...
        for (int i = 0; i < length; i++) {
            int random = (int) (Math.random() * 89 * by) + 10 * by;
            if (i % 2 == 0) {
                random = -random;
            }
            arr[i] = random;
     ...
     * @return int[]
     */
    public static Integer[] getDataIntegerNumber(int size, int number) {
        Integer[] arr = new Integer[size];
        int length = arr.length;
        if (number == 1) {
            Random random=new Random();
     ...
     * @return long[]
     */
    public static long[] getDatalong(int size) {
        long[] arr = new long[size];
        int length = arr.length;
        Random random = new Random();
        for (int i = 0; i < length; i++) {
            long i1 = random.nextInt(length);
            long i2 = random.nextInt(length);
            arr[i] = i1 - i2;
        }
    ...
    /**
     * 功能：随机产生年龄18~60岁
     * @param size 100*100(长*宽) x代表行[0] ,y列[1]
     * @return
     */
    public static  int[][] getMatrix(String size) {
    ...
    /**
     * @param size 获取数量
     * @TODO 随机获取 国家-省-市-(县或区) 地址
     */
    public static List<String> getSite(int size) {
    ...
    public static String getSite() {
        ...
        String country = "中国";
        List<String> list = new ArrayList<>();
        List<String> provinces = instance.getProvinces(country);
        String province = "";
        if (provinces.size() > 0) {
            int provincesIndex = new Random().nextInt(provinces.size());
            province = provinces.get(provincesIndex);    //获取省
        }
        String citie = "";
        List<String> cities = instance.getCities(country, province);
        if (cities.size() > 0) {
            int citiesIndex = new Random().nextInt(cities.size());
            citie = cities.get(citiesIndex);//获取市
        }


        List<String> getcountys = instance.getcounty(country, province, citie);
        String getcounty = "";
        if (getcountys.size() > 0) {
            int getcountyIndex = new Random().nextInt(getcountys.size());
            getcounty = getcountys.get(getcountyIndex);
        }
        String site = country + "-" + province + "-" + citie + "-" + getcounty;
        return  site;
    }
    /**
     *
    ...
    public static List<UserData> getUserDataList(int size) {
        ...
        for (int i = 0; i < size; i++) {
            Random random=new Random();
            int i1 = random.nextInt(size * 10);
            UserData a1 = UserData.builder()
                    .id(i1)
                    .name(randomName(true,3))
                    .pass(HashUtil.md5(getRandomJianHan(10)))
                    .age(getAge())
                    .sex(getSex())
                    .site(getSite())
                    .build();
    ...
    public static UserData[] getUserDataArray(int size) {
        UserData[] userData=new UserData[size];
        for (int i = 0; i < size; i++) {
            Random random=new Random();
            int i1 = random.nextInt(size * 10);
            UserData a1 = UserData.builder()
    ...
    public static UserData getUserOne() {
        UserData a1 = UserData.builder()
                .id(0)
                .name(randomName(true,3))
                .pass(HashUtil.md5(getRandomJianHan(10)))
