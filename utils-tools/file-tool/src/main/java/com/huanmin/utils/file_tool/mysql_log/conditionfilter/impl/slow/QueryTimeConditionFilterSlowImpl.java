package com.huanmin.utils.file_tool.mysql_log.conditionfilter.impl.slow;


import com.huanmin.utils.file_tool.mysql_log.conditionfilter.ConditionFilterSlow;
...
@Controller
public class QueryTimeConditionFilterSlowImpl implements ConditionFilterSlow {

    
    @Override
    public Map<String, List<SlowEntity>> condition(Map<String, List<SlowEntity>> map, SlowEntity slowEntity) {
        if (slowEntity.getQueryTime()>0) {
            ...
            for (Map.Entry<String, List<SlowEntity>> stringListEntry : map.entrySet()) {
                 List<SlowEntity> collect = stringListEntry.getValue().stream().filter((data) -> {
                    return data.getQueryTime()>=slowEntity.getQueryTime();
